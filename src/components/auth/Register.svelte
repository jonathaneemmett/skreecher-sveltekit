...
<form action="?/register" method="POST" use:enhance={({ cancel }) => {
    ...
    <div>
        <input type="text" name="handle" id="handle" placeholder="@handle" bind:value={handle}>
        {#if handleError}
            <div class="error">Handle is required.</div>
        {/if}
    </div>
    <div>
    ...
    {/if}
    {#if form?.handle}
        <div class="error">Handle is already in use.</div>
    {/if}
    {#if form?.server}
        <div class="error">There was a server problem, please try again later.</div>
    {/if}
    <div>
